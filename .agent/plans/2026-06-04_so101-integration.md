...
> ### 📍 実装状況サマリ（2026-06-10 更新）
> **SO-101 実機が動く状態に到達。** 運用・トラブルシュートは skill **`so101-operate`** が正本。
> - **Phase 0/1/2: 完了** — so101 制御層・FK/IK/safety・MuJoCo sim 済み。UI/サーバ統合は
>   `RobotConfig` 全面注入ではなく **`So101Subsystem`（server.py）+ ui.html SO-101 タブ** で実現
>   （myCobot 経路は無改変）。`/so101/*` API。
> - **Phase 3: 実質完了（設計変更あり）** — `SO101Hub`(HubBase) は**作らず**、軽量な
>   `LerobotSo101Driver` + `So101Controller`（speed ペーシング/abort）+ `So101Subsystem` で代替。
> - **Phase 4: 実機ブリングアップ完了** — 組立→ID設定→キャリブ(GUI)→read/write/release/jog/HOME
>   全て実機検証。残: 手首カメラ hand-eye 再キャリブ・reach-grid 再計算は未着手。
> - **残課題**: `profile.SAFETY.floor_z_mm` 暫定0の実測校正 / 12V **5A** 電源 / IK 非決定性 /
>   キャリブ script の `so101_tools/` 整理。詳細は [memory/so101_bringup.md](../../.claude/memory/so101_bringup.md)。
...
- [x] **MuJoCo 仮想アーム**（`sim/mujoco_sim.py`）：公式 MJCF+メッシュを vendoring、Windows で
      オフスクリプト描画も動作。自作 FK を MuJoCo body 位置と突き合わせ **誤差<0.01mm**（独立検証）
- [x] **実リンク GLB**（`build_glb.py` → `meshes_glb/`）：URDF visual を per-link で GLB 化、検証済み
      （Phase 2 で ui.html に GLTFLoader で配線。リンク→FKフレーム対応は test に明記）
- [x] **Feetech/lerobot ドライバ雛形**（`driver.py`）：lerobot ラップ（lazy）+ オフライン Mock + 並進ヘルパ、10 テスト緑
...
- [x] poses・gestures（5-DoF 版）（`poses.py`/`gestures.py`、全 safety 通過 + MuJoCo 目視確認、9 テスト緑）
      ※OBSERVE 系はカメラ実装依存で暫定
- [x] 制御層 `controller.py`（safety検証→経路計画→逐次実行、driver 非依存）+ MuJoCo 実行デモ
...
## 8. オープンソース活用（2026-06-04 調査 → 採用）

SO-101 がオープン HW/SW である強みを、わたしたちの文脈（自前スタック・Windows・
学習は当面なし・ハード前開発）で最大化する判断。

| 資産 | 採用 | 効果 |
|---|---|---|
| 公式 **MuJoCo モデル**（MJCF+メッシュ、Apache-2.0） | **採用・vendoring** | Windows で動く仮想アーム。FK の独立検証(<0.01mm)、ハード前の動作確認基盤 |
| **実リンクメッシュ**（STL→GLB） | **採用** | three.js UI を実形状化（Phase 2 で配線）。MuJoCo 描画で形状は検証済み |
| **lerobot 制御 API**（`SO101Follower`/`FeetechMotorsBus`） | **ラップ採用** | Feetech シリアルを自前実装せず lerobot に委譲。LeRobot 形式互換を維持 |
| データセット/事前学習ポリシー（SmolVLA 等） | **保留（互換維持）** | 制御層を LeRobot 形式互換に保ち、将来の学習路を開けたまま温存 |
| `lerobot-kinematics`（Placo/Pinocchio IK） | **見送り** | Windows インストールが地獄。考え方のみ参照、必要なら PyRoki |
| ROS2 / Isaac / Genesis | **見送り** | 自前スタックあり／GPU+Linux 前提 |

実機到着前の開発substrate は **MuJoCo**。実機リスクの本丸は **Feetech ドライバ**（`driver.py` 雛形済み、
実機検証は未）。
